"""Weather service: current weather + daily forecast via Open-Meteo.
...
Flow:
  ...
  3. Format result for the LLM context / frontend card
"""
...
async def _fetch_weather(lat: float, lon: float) -> dict[str, Any]:
    """Query Open-Meteo for current weather + today's forecast."""
    ...
    return {
        ...
        "today": {
            ...
            "condition": _condition((daily.get("weather_code") or [None])[0]),
...
async def weather_search(
    ...
    Returns dict with: place, display_name, lat, lon, current, today.
    """
    lat: float | None = None
    lon: float | None = None
...
def weather_result_to_text(data: dict[str, Any]) -> str:
    ...
    wind = f"{c['wind_dir_text']} {c['wind_speed']}km/h" if c.get("wind_speed") is not None else "无"
    lines.append(f"风: {wind}")
    if c.get("precipitation") is not None:
        lines.append(f"降水: {c['precipitation']}mm")
    lines.append(f"今日: {t['condition']}, 最高 {t['temp_max']}°C / 最低 {t['temp_min']}°C")
    return "\n".join(lines)